...
class PLL():
	def __init__(self,
		...
		self.vco_freq = vco_free
		self.vco_phase = 0
		self.K_vco = K_vco
		self.amplitude = amplitude

	def forward_VCO(self, input, duration):
		prev = self.vco_freq		
		self.vco_freq = self.vco_freq + self.K_vco*input*duration
		self.vco_phase = self.vco_phase + (self.vco_freq + prev)/2*duration		
